...
export class UsersService {
  ...
  public async confirmEmail(
    ...
    if (user.confirmed) {
      if (user.email === email) throw new BadRequestException('Email already confirmed');;

      await user.oauthProviders.load();
      const nonLocalProviders = user.oauthProviders.getItems().filter(
        (provider) => provider.provider !== OAuthProvidersEnum.LOCAL
      );

      if (nonLocalProviders.length > 0) {
        await this.commonService.removeEntities(nonLocalProviders);
        user.oauthProviders.remove(nonLocalProviders);
      }
  ...
  public async updateEmail(
    ...
    if (!user.password || user.password === 'UNSET') {
      throw new BadRequestException('You need to update your password before performing this operation');
    }
    ...
    return { message: `A confirmation email has been sent to ${formattedEmail}` };
  }

  public async delete(userId: number, dto: PasswordDto): Promise<UserEntity> {
    const user = await this.findOneById(userId);

    if (user.password !== 'UNSET' && !(await compare(dto.password, user.password))) {
      throw new BadRequestException('Wrong password');
    }
  ...
  public async findOrCreate(
    ...
    if (
      ...
      await this.confirmEmail(user.id, user.credentials.version, email);
      await this.createOAuthProvider(provider, user.id);
    }
